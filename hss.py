...
class HSS:
    ...
    def build(self):
        ...
        for l in range(self.Partition.max_level, 0, -1):
            ...
            for obj in self.Partition.level_to_nodes[l]:
                rows_ind = obj.i_row
                columns_ind = [] if obj.is_root else functools.reduce(operator.add, [t.i_col for t in obj.get_N()])
                ...
                rows_ind = [] if obj.is_root else functools.reduce(operator.add, [t.i_row for t in obj.get_N()])
