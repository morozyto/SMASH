...
def get_G(k, i, level_to_nodes, max_level, b):
    ...
    if k == max_level:
        ...
        #print(f'morozyto debug {k}, {i} {tools.print_matrix(np.transpose(level_to_nodes[k][i].Ws[0]))}')
        ...
        res = t + s
    #print(f'returning get_G {k}, {i} : {tools.print_matrix(res)}')
    return res
...
def get_F(k, i, level_to_nodes, max_level, A, b):
    ...
    #print(f'returning get_F {k}, {i} : {tools.print_matrix(np.array(res).reshape(len(res), 1))}')
    return res
